...
import numpy as np
import wandb
from model_vanilla import VModel
...
def continual_learning(args, van, wandb):
    ...
    if args.resume_full_path is not None:
        ...
        latent_dict, rehearsal_ixs, class_id_to_item_ix_dict = van.resume(args.base_init_classes, args.resume_full_path)
        ...
        probas_base, true_base = van.predict(init_test_loader)
        ...
        probas_base, true_base = van.predict(test_loader_old)
        top1_base, top5_base = utils.accuracy(probas_base, true_base, topk=(1, 5))
    ...
    for class_ix in range(args.cl_min_class, args.cl_max_class, args.class_increment):
        ...
        latent_dict, rehearsal_ixs, class_id_to_item_ix_dict = van.update_buffer(curr_loader, latent_dict,
        ...
        if num_session > 1:
            ...
            top1_base, top5_base = utils.accuracy(probas_base, true_base, topk=(1, 5))
            ## Prev (73)
            probas_prev, true_prev = van.predict(test_loader_prev)
            ...
            ## new (73)
            probas_new, true_new = van.predict(test_loader_new)
            ...
            ## OLD Classes
            probas_old, true_old = van.predict(test_loader_old)
            top1_old, top5_old = utils.accuracy(probas_old, true_old, topk=(1, 5))
            ## NEW Classes
            probas_new, true_new = van.predict(test_loader_new)
            true_new = true_new + args.base_init_classes # new lables
        ...
        if num_session == 1:
            val_acc_all, val_acc_old, val_acc_new, val_acc_base = van.train_s1(train_loader_new, train_loader_old,
             test_loader_new, test_loader_old, num_iter, val_acc_all, val_acc_old, val_acc_new, val_acc_base, wandb)
        else:
            val_acc_all, val_acc_old, val_acc_new, val_acc_base = van.train(train_loader_new, train_loader_old,
    ...
    if not os.path.exists(exp_dir):
        os.makedirs(exp_dir)
    np.save(os.path.join(exp_dir, 'val_all_van.npy'), val_acc_all)
    np.save(os.path.join(exp_dir, 'val_old_van.npy'), val_acc_old)
    np.save(os.path.join(exp_dir, 'val_new_van.npy'), val_acc_new)
    np.save(os.path.join(exp_dir, 'val_base_van.npy'), val_acc_base)
...
if __name__ == '__main__':
    ...
    wandb.init(name='Vanilla_CL_' + str(args.seed),
    ...
    van = VModel(num_classes=args.num_classes, classifier_G=args.base_arch,
    ...
    ## Continual Learning
    continual_learning(args, van, wandb)
