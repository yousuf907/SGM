...
class Block(nn.Module):
    ...
    """
    def __init__(self, dim, drop_path=0., rank=48):
        super().__init__()
        self.dwconv = nn.Conv2d(dim, dim, kernel_size=7, padding=3, groups=dim) # depthwise conv
        self.norm = LayerNorm(dim, eps=1e-6)
        #self.pwconv1 = nn.Linear(dim, 4 * dim) # pointwise/1x1 convs, implemented with linear layers
        self.pwconv1 = lora.Linear(dim, 4 * dim, r=rank)
        self.act = nn.GELU()
        self.grn = GRN(4 * dim)
        #self.pwconv2 = nn.Linear(4 * dim, dim)
        self.pwconv2 = lora.Linear(4 * dim, dim, r=rank)
...
class ConvNeXtV2(nn.Module):
    ...
    def __init__(self, in_chans=3, num_classes=1000, 
        ...
        cur = 0
        #ranks = [16,16,48,48] ##rank=ranks[i]
        for i in range(4):
            stage = nn.Sequential(
...
### ----- ###
### block 2 : 14 x 14 x 192 /// extract_features_from: model.downsample_layers.2 ///
### ----- ###
...
class ConvNeXt_block2(nn.Module):
    def __init__(self, pretrained_model=None, num_classes=None):
        ...
        if pretrained_model is not None:
            safe_load_dict(self.model, pretrained_model, should_resume_all_params=False)
...
### ----- ###
### block 3 : 7 x 7 x 384 /// extract_features_from: model.downsample_layers.3 /// 
### ----- ###

class ConvNeXt_block3(nn.Module):
    def __init__(self, pretrained_model=None, num_classes=None):
        super(ConvNeXt_block3, self).__init__()
    ...
    def get_feature(self, x): # x dim: # N x 384 x 7 x 7
        x = self.forward_features(x)
        return x
